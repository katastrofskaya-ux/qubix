...
RULES = [
    ...
    (STOP, r"круглосуточн|24\s*/\s*7|24 часа в сутки"
           r"|с \d{1,2}[:.]\d{2}\s*(?:до|[–—-])\s*\d{1,2}[:.]\d{2}"
           r"|\d{1,2}[:.]\d{2}\s*[–—-]\s*\d{1,2}[:.]\d{2}\s*(?:по\s*)?МСК"
           r"|(?:команда|люди|поддержка|саппорт)\s+на связи с \d",
     "часы работы поддержки или команды",
     "режим работы во внешних текстах не называем вообще — ни «круглосуточно», ни «24/7», ни «с 9:00 до 21:00»; пишем «поддержка на связи» без времени",
     "Анастасия 16.09.2026, со ссылкой на многократное решение владельца; действует до её отмены", None),
    ...
    (STOP, r"медиабаинг целиком|от залива до созвонов и финанс",
     ...
     "MARKETING-70 §1, решение владельца 10.09.2026", None),
