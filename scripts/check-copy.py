...
Источники правил: BRAND-1 §9-10 (стоп-лист), BRAND-4 / docs/tone-of-voice.md,
...
WHITELIST = [
    ...
    "от задачи до созвона",               # слоган BRAND-1 / CLAUDE.md
]

RULES = [
    # (уровень, регулярка, что нашли, чем чинить, источник, флаг-исключение)
    (STOP, r"с \d{1,2}[:.]\d{2}\s*(?:до|[–—-])\s*\d{1,2}[:.]\d{2}"
           r"|\d{1,2}[:.]\d{2}\s*[–—-]\s*\d{1,2}[:.]\d{2}\s*(?:по\s*)?МСК"
           r"|(?:команда|люди|поддержка|саппорт)\s+на связи с \d"
           r"|\bМСК\b",
     "часы работы команды или упоминание МСК",
     "часы живой команды и «МСК» во внешних текстах не называем: строка про саппорт — «бот @qubix_pro_bot отвечает круглосуточно», без времени и без часового пояса",
     "решение владельца 04.09.2026, подтверждено админом 09.09; подтверждено Анастасией 16.09", None),

    (STOP, r"медиабаинг целиком|от залива до созвонов и финанс",
     "старая главная формула (снята владельцем 10.09.2026)",
     "новая формула: «Свой сервер — своя ракета! Тянет от залива до офисного пакета.» (с именем «Qubix:» — для холодной аудитории); слово «медиабаинг» из формул убрано",
     "BRAND-1 §1, решение владельца 10.09.2026", None),

    (STOP, r"уровня Claude|аналог\w* Claude",
     "занижение «уровня/аналог Claude»",
     "у нас стоит сам Claude: канон «Сам Claude — на всю команду одной подпиской, без наценки сверху»",
     "решение владельца 04.09, повторено 09.09 (CONTENT-39 #3, CONTENT-41 #9)", None),

    (WARN, r"подписк\w[^.\n]{0,80}(на вашем ключе|по цене поставщика)|(на вашем ключе|по цене поставщика)[^.\n]{0,80}подписк\w",
     "возможная склейка двух путей Claude (подписка + свой ключ) в одной фразе",
     "подписка Claude и свой ключ OpenRouter — два разных пути: подписка канонной строкой, ключ — отдельной фразой",
     "решение владельца 04.09, перечень носителей — CONTENT-46 коммент 09.09", None),
    ...
    (STOP, r"всё в одном|все в одном|all-in-one",
     ...
     "BRAND-1 §5 и стоп-лист п.1", None),
    ...
    (STOP, r"Keitaro|Кейтаро|Binom|Бином|Voluum|RedTrack|adset\.pro|AdsPower|Dolphin|Octo Browser|Multilogin|\bAIO\b",
     ...
     "BRAND-1 стоп-лист п.2", "author_voice"),
    ...
    (STOP, r"в отличие от трекер|лучше любого трекера|не просто трекер|не трекер, а|обычные трекеры",
     ...
     "BRAND-1 §6, docs/tone-of-voice.md п.8", None),
    ...
    (WARN, r"облак\w+ (видит|смотрит|хранит|доит)|сервис\w* (доя|забира|держ)|поставщик (видит|забирает|держит)|чужие держат",
     ...
     "BRAND-1 стоп-лист п.2а и §9", "author_voice"),
    ...
    (WARN, r"(?<![\d+])\b(\d{2,4})\s+(настро|шаблон|метрик|инструмент|экран|пол(е|я|ей))",
     ...
     "BRAND-1 §9, решение владельца 19.08.2026", None),
        ...
        for w in YO_CHECK:
            if re.search(r"(?<![\w])" + w + r"(?![\w])", line, re.I):
                findings.append((WARN, n, w, "буква «ё» уронена в «е»",
                                 f"написать «{YO[w]}»", "BRAND-1 §9", raw.strip()))
                break
    return findings
