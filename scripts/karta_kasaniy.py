...
T_EXPIRING = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
              ...
              "Расскажите, как идёт работа: сервер развернули, первую связку запустили, конверсии приходят? Если где-то застряли — напишите, разберу лично или подключу команду.\n\n"
              "Продление — в вашем кабинете my.qubix.pro, оплата в USDT.{disc} Если остались вопросы по тарифу или счёту — задайте их мне напрямую, отвечу.")
T_EXPIRED = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
             "Ваш пробный месяц закончился {ago}. Хочу узнать ваше мнение о продукте: что понравилось, чего не хватило, что остановило от продления. Это поможет нам сделать Qubix лучше, а вам — получить ответ на любой открытый вопрос от меня напрямую.\n\n"
             "Если решите продолжить — данные и настройки на вашем сервере сохранены, доступ восстанавливается оплатой в кабинете my.qubix.pro. С любым шагом помогу.")
...
for c in clients:
    ...
    if n in SKIP:
        ...
        disc = " По вашему коду действует скидка 50%." if DISCOUNT.get(c["code"], 0) == 50 else ""
