...
RELEASE_BASE = ("Здравствуйте{name}! Это Анастасия, CEO Qubix. Мы с вами общались {when}, и я обещала написать, когда выйдет большой релиз.\n\n"
                "Он вышел 28 августа: Qubix стал супераппом. Документы и таблицы команды теперь живут прямо в панели, а рекламным кабинетом можно управлять скриптами. "
                "Всё по-прежнему стоит на вашем сервере, данные остаются у вас. Продукт — qubix.pro, полный список изменений — docs.qubix.pro/ru/changelog/overview\n\n"
                "{personal}\n\n"
...
T_EXPIRING = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
              "Пишу заранее: {date} у вас заканчивается пробный месяц.{disc}\n\n"
              "Заодно: 28 августа у нас вышел большой релиз — Qubix стал супераппом, документы и таблицы команды теперь живут прямо в панели. Продукт — qubix.pro, что появилось в релизе — docs.qubix.pro/ru/changelog/overview\n\n"
              "Расскажите, как вам месяц в Qubix: что отметили для себя, с чем столкнулись в работе? Успели попробовать документы и таблицы прямо в панели?")
T_EXPIRED = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
             "Ваш пробный месяц закончился {ago}. Данные и настройки на вашем сервере сохранены — вернуться можно в любой момент.\n\n"
             "А 28 августа вышел большой релиз: Qubix стал супераппом, документы и таблицы команды теперь живут прямо в панели. Продукт — qubix.pro, что появилось в релизе — docs.qubix.pro/ru/changelog/overview\n\n"
             "Расскажите, как вам месяц в Qubix: что отметили для себя, с чем столкнулись, что остановило? Успели попробовать документы и таблицы прямо в панели?")
T_STUCK_NEW = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
               "Вы зарегистрировались у нас {ago}, но до установки на сервер пока не дошли. Установка занимает около двадцати минут: одна команда на вашем сервере, дальше мастер в браузере. Первый месяц бесплатный, со всеми возможностями. Продукт — qubix.pro, что вышло в августовском релизе — docs.qubix.pro/ru/changelog/overview\n\n"
...
T_STUCK_PLATFORM = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
                    "Вы пришли к нам через {platform} и зарегистрировались, но до установки на сервер пока не дошли. Установка занимает около двадцати минут: одна команда на вашем сервере, дальше мастер в браузере. За вашим аккаунтом закреплена скидка 50% от {platform} — условие периода Early Bird: пока он идёт, процент закрепляется за вами на год с первой оплаты. Продукт — qubix.pro, что вышло в августовском релизе — docs.qubix.pro/ru/changelog/overview\n\n"
                    "Подскажите, что остановило: нет подходящего сервера, вопросы по домену или просто не хватило времени?")
T_STUCK_OLD = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
               "Вы регистрировались у нас в {month}, до установки на сервер тогда не дошло. С тех пор вышел большой релиз — Qubix стал супераппом, теперь документы и таблицы команды живут прямо в панели. Продукт — qubix.pro, что появилось в релизе — docs.qubix.pro/ru/changelog/overview Бесплатный месяц у вас остаётся.\n\n"
...
T_PAYING = ("Здравствуйте! Это Анастасия, CEO Qubix.\n\n"
            "Вы с нами с июля, и мне важно, чтобы у вас всё работало как надо. Что вышло в релизе 28 августа — docs.qubix.pro/ru/changelog/overview, сам продукт — qubix.pro\n\n"
...
T_BOT = ("Здравствуйте{name}! Меня зовут Анастасия, я CEO Qubix.\n\n"
         "Вы {when} обращались к нашему боту и смотрели демо, но аккаунт пока не завели. Qubix ставится на ваш сервер, первый месяц бесплатный со всеми возможностями. Продукт — qubix.pro, что вышло в августовском релизе — docs.qubix.pro/ru/changelog/overview\n\n"
         "Подскажите, какую задачу решаете: трекер, PWA-витрины, автоправила для Facebook, командная работа?")
T_SILENT = "Здравствуйте! Это Анастасия, Qubix. Больше беспокоить не буду — если решите вернуться, напишете мне?"
...
for c in clients:
    ...
    if n in SKIP:
        ...
        disc = (" Скидка 50% по вашему коду — условие периода Early Bird: пока он идёт, процент закрепляется за вами на год с первой оплаты."
                if DISCOUNT.get(c["code"], 0) == 50 else "")
