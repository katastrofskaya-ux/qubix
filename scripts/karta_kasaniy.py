...
T_STUCK_PLATFORM = ("Здравствуйте! Меня зовут Анастасия, я CEO Qubix.\n\n"
                    "Вы пришли к нам через {platform} и зарегистрировались, но до установки на сервер пока не дошли. Установка занимает около двадцати минут: одна команда на вашем сервере, дальше мастер в браузере. За вашим аккаунтом закреплена скидка 50% от {platform} — стартовое условие: пока оно действует, процент закрепляется за вами на год с первой оплаты.\n\n"
                    "Подскажите, что остановило: нет подходящего сервера, вопросы по домену или просто не хватило времени?")
...
for c in clients:
    ...
    if n in SKIP:
        ...
        disc = (" Скидка 50% по вашему коду — стартовое условие: пока оно действует, процент закрепляется за вами на год с первой оплаты."
